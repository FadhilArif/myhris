...
export async function openPayrollPreflight(runId){
  const [runs,slips,emps,components]=await Promise.all([sbAll('payroll_runs',{eq:{id:runId}}),sbAll('payslips',{eq:{payroll_run_id:runId}}),sbAll('employees'),sbAll('payroll_components')]);
  const run=runs[0];if(!run){showToast('Periode payroll tidak ditemukan.',true);return;}
  const active=emps.filter(e=>e.employment_status==='active');
  const missingPtkp=active.filter(e=>!e.ptkp_status).length;
  const missingSalary=active.filter(e=>!(Number(e.basic_salary)>0)).length;
  const missingBank=active.filter(e=>!e.bank_name||!e.bank_account_number).length;
  const negativeNet=slips.filter(s=>Number(s.net_salary)<0).length;
  const required=['bpjs_kesehatan','bpjs_jht','bpjs_jp','pph21'];
  const available=new Set(components.map(c=>c.calc_type).filter(Boolean));
  const missingComponents=required.filter(x=>!available.has(x));
  const checks=[
    ['Jumlah slip vs karyawan aktif',slips.length===active.length,slips.length+'/'+active.length],
    ['PTKP karyawan',missingPtkp===0,missingPtkp+' belum diisi'],
    ['Gaji pokok',missingSalary===0,missingSalary+' belum valid'],
    ['Data bank/rekening',missingBank===0,missingBank+' belum lengkap'],
    ['Gaji bersih tidak negatif',negativeNet===0,negativeNet+' slip negatif'],
    ['Komponen BPJS & PPh 21',missingComponents.length===0,missingComponents.length?missingComponents.join(', '):'lengkap']
  ];
  const rows=checks.map(x=>'<tr><td>'+escapeHtml(x[0])+'</td><td>'+statusBadge(x[1]?'approved':'warning')+'</td><td>'+escapeHtml(x[2])+'</td></tr>').join('');
  openModal('<div style="width:min(760px,calc(100vw - 32px));max-height:90vh;overflow:auto;"><h3>Payroll Preflight Check</h3><div style="font-size:12px;color:var(--text-muted);margin-bottom:12px;">Pemeriksaan internal sebelum review/approval. Ini adalah validation check MyHRIS, bukan pernyataan kepatuhan pajak/hukum.</div><div class="card" style="padding:0;overflow:auto;"><table><thead><tr><th>Pemeriksaan</th><th>Status</th><th>Detail</th></tr></thead><tbody>'+rows+'</tbody></table></div><div style="display:flex;justify-content:flex-end;margin-top:12px;"><button class="btn btn-outline" onclick="closeModal()">Tutup</button></div></div>');
}
