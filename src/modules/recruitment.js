...
export async function closeJobPosting(jobId){
  ...
  showToast('Lowongan ditutup.'); renderRecruitment();
}
...
export async function deleteJobPosting(jobId){
  if(!confirm('Hapus lowongan ini beserta seluruh data kandidat? Tindakan ini tidak bisa dibatalkan.')) return;
  const delCand = await sb.from('candidates').delete().eq('job_posting_id', jobId);
  if(delCand.error){ showToast('Gagal menghapus kandidat: '+delCand.error.message, true); return; }
...
export async function convertCandidate(candidateId, applicantId){
  ...
  if(applicantId){
    ...
    if(existingProfile){ await sb.from('profiles').update({ employee_id: newEmp.id, role: 'employee' }).eq('id', applicantId); }
  }
  ...
  if(templates.length){
    ...
    const { data: newOnb } = await sb.from('onboardings').insert({ employee_id: newEmp.id, template_id: template.id, started_date: new Date().toISOString().slice(0,10), status: 'in_progress' }).select().single();
    if(newOnb && items.length){
      await sb.from('onboarding_items').insert(items.map(it => ({
        onboarding_id: newOnb.id, task_name: it.task_name, owner_role: it.owner_role,
        due_date: new Date(Date.now() + it.due_days * 86400000).toISOString().slice(0,10), is_done: false
      })));
    }
  }
  showToast(`✅ ${empData.full_name} berhasil dikonversi jadi karyawan!`);
  closeModal(); renderRecruitment();
}
